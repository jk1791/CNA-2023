    ...
    for m in range(num_scales):
        ...
        for j in range(q_num):
            ...
            if q_act == 0:
                q_act += q_step / 4.0
            if flag == False :
                fluctfunct[2,m,j] = np.mean(abs(segment_var[2])**(q_act / 2))
            else:
                fluctfunct[2,m,j] = np.mean(np.sign(segment_var[2]) * (abs(segment_var[2]))**(q_act / 2))

#            if j == 0 and m == num_scales - 1:
#                print (segment_var[2])
#                print (abs(segment_var[2])**(q_act / 2))

            fluctfunct[2,m,j] = np.sign(fluctfunct[2,m,j]) * abs(fluctfunct[2,m,j])**(1.0 / q_act)

            fluctfunct[0,m,j] = np.mean(segment_var[0]**(q_act / 2))
            fluctfunct[0,m,j] = fluctfunct[0,m,j]**(1.0 / q_act)
            fluctfunct[1,m,j] = np.mean(segment_var[1]**(q_act / 2))
            fluctfunct[1,m,j] = fluctfunct[1,m,j]**(1.0 / q_act)
...
def output1():
    ...
    with open(outfile, 'w') as file:
        for j in range(q_num):
            ...
            for m in range(num_scales):
                window_lngth = round(np.exp(np.log(min_scale) + m * logwin_step))
                file.write(f'{q_act} {window_lngth} {fluctfunct[2,m,j]}\n')
            file.write('\n')
    ...
    with open(outfile, 'w') as file:
        for j in range(q_num):
            ...
            for m in range(num_scales):
                window_lngth = round(np.exp(np.log(min_scale) + m * logwin_step))
                file.write(f'{q_act} {window_lngth} {fluctfunct[0,m,j]}\n')
            file.write('\n')
    ...
    with open(outfile, 'w') as file:
        for j in range(q_num):
            ...
            for m in range(num_scales):
                window_lngth = round(np.exp(np.log(min_scale) + m * logwin_step))
                file.write(f'{q_act} {window_lngth} {fluctfunct[1,m,j]}\n')
            file.write('\n')
...
# input files
infiles.append("Kaskada_LN_l120_n10_1.dat")
infiles.append("Kaskada_LN_l120_n10_1.dat")
# time series column
column = 1
...
fluctfunct = np.zeros(((3,num_scales,q_num)))
fluctfunct = fluctuation_function(flag)
output1()
